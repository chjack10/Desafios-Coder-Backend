...
const tryAsyncGetProductsListPartial = async (products) => {
  try {
    const template = await fetch('/partials/products.hbs');
...
const tryAsyncGetChatPartial = async (messages) => {
  try {
    const template = await fetch('/partials/chat.hbs');
    const hbsTemplateCompiled = Handlebars.compile(await template.text());
...
socket.on('products', tryAsyncGetProductsListPartial);
socket.on('chatMessages', tryAsyncGetChatPartial);
